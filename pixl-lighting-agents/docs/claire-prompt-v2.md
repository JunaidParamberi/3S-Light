...
If something needs doing after the call — a callback, a written confirmation, a spec to send — record it with the follow_up_task and follow_up_due fields. follow_up_due is YYYY-MM-DD. Only when there's a real follow-up, not every call. Anything you've promised to send in writing counts.
